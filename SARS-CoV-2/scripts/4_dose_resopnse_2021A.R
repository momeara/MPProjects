...
viral_intensity_scores <- image_scores_CX5_2021A %>%
    dplyr::group_by(
         Hydroxychloroquine_Concentration,
         Lactoferrin_Concentration,
         row,
...
load("intermediate_data/rf_scores_field_2021A.Rdata")
rf_scores <- rf_scores_field_2021A %>%
...
well_scores <- meta_scores %>%
    dplyr::left_join(
        ...
        dplyr::select(
            ...
            infected_count_mean,
            infected_count_sem),
        by=c("Hydroxychloroquine_Concentration", "Lactoferrin_Concentration")) %>%
    dplyr::left_join(
        rf_scores %>%
        dplyr::select(
            Hydroxychloroquine_Concentration,
...
well_scores %>% readr::write_tsv("product/plate_1999B_Lf_vs_Hcq_well_scores.tsv")
        ...
        plot <- plot +
            ggplot2::geom_errorbar(
               mapping=ggplot2::aes(
                 ...
                 group=Hydroxychloroquine_Concentration),
...
p <- make_dose_response_plot(
    ...
    score_types=c(
        ...
        viral_intensity="red"))
ggplot2::ggsave(
    "product/figures/dose_response/plate_1999B/plate_cell_count_nucleoli_count_viral_intensity_dose_response_lf_by_hcq_200505.pdf",
    width=6, height=6)
...
hcq_area <- tibble::tibble(
  ...
  dplyr::mutate(
    label=well_scores$rem_la

    c("0", "3.2", "6.2", "12.4", "25", "48", "100", "200"),
