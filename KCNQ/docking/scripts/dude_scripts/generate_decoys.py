...
def query_zinc(lig_id, smiles, prop_range_dict, ligand_file):
	...
	for i in range(7):
		...
		command3 = 'curl "http://zinc15.docking.org/protomers/subsets/usual.txt:smiles+zinc_id+mwt+logp+rb+hbd+hba+net_charge+prot_id+model_type_name" -F "mwt-between=%d %d" -F "logp-between=%f %f" -F "sort=no" -F "net_charge-between=%d %d" -F "count=%d"' % (mwt_low, mwt_high, logp_low, logp_high, int(chg_low), int(chg_high), query_num)
